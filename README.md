...
> 📘 完整部署教學 + 使用手冊，也有網頁版：[`docs/guide.html`](docs/guide.html)（部署到 GitHub Pages 後可透過 `.../guide.html` 開啟）。

這是**獨立的新專案**（新 GitHub repo `jioka2`），舊版 `jioka`（試算表版）原封不動、繼續運作，兩者資料庫互不相通。

## ✨ 功能特色

跟一代完全相同的功能（月曆投票、地點票選、留言、主揪/次管理者、再揪一次、我的揪咖紀錄、結果卡、管理後台、CSV 匯出），差別只在後端架構與同步速度：
...
3. **建立 Firestore + 部署規則**：左側「Firestore Database」→ 建立資料庫（正式版模式）→「規則」分頁貼上整份 [`firestore.rules`](firestore.rules) → 發布
4. **設定前端**：編輯 `docs/config.js`，填入步驟 1 的 `firebaseConfig` 與步驟 2 的 Client ID
5. **部署到 GitHub Pages**：推到新的 GitHub repo → Settings → Pages → Branch 選 `main`、資料夾選 `/docs`
